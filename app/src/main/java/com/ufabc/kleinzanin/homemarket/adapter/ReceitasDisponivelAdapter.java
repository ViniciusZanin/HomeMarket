...
/**
 * Created by Felippe on 30/04/2015.
 */
public class ReceitasDisponivelAdapter extends BaseAdapter{
    private ReceitasDAO dao;
    private Context context;
    ArrayList<Receitas> receitas;
    LayoutInflater inflater;

    public ReceitasDisponivelAdapter(Context c){
        this.context = c;
        this.dao = ReceitasDAO.newInstance(c);
        inflater = (LayoutInflater ) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        receitas = dao.receitas_disp();
    }

    public ReceitasDisponivelAdapter(ReceitasDetailFragment receitasDetailFragment) {
